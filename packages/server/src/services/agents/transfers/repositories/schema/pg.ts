...
export async function up(db: Kysely<any>): Promise<void> {
  try {
    await db.schema
      ...
      .addColumn('tx_primary', 'bytea')
      .addColumn('tx_secondary', 'bytea')
      .addColumn('tx_index', 'integer')
      .addColumn('tx_module', 'text')
      .addColumn('tx_method', 'text')
      .addColumn('tx_signer', 'bytea')
    ...
    await db.schema
      ...
      .addColumn('snapshot_start', 'bigint', (cb) => cb.notNull())
      .addColumn('snapshot_end', 'bigint', (cb) => cb.notNull())
      .execute()
    ...
    await db.schema
      .createIndex('ic_transfers_type_network_usd_sent_at_id_index')
      .ifNotExists()
      .on('ic_transfers')
      .columns(['type', 'network', 'usd', 'sent_at', 'id'])
      .execute()
