...
import { BalanceUpdateItem, CustomDiscoveryFetcher, RuntimeQueueData } from '../types.js'
...
export function hydrationBalancesFetcher(
  ...
  return async (account: HexString) => {
    const ss58 =
      account.length === 42 ? publicKeyToSS58(padAccountKey20(account)) : publicKeyToSS58(fromHex(account))
