...
describe('Bitcoin watcher', () => {
  describe('re-orgs', () => {
    it('should handle a canonical chain', async () => {
      ...
      expect((await heads.get('00000000000000000002'))?.hash).toBe('0xC2')
    })
    ...
    it.skip('should prune heads cache', async () => {
      ...
      expect((await heads.get('00000000000000000504'))?.hash).toBe('0xC504')
    })
    ...
    it('should handle a 50 blocks re-org', async () => {
      ...
      expect((await heads.get('00000000000000000025'))?.hash).toBe('0xC25')
    })
    ...
    it('should handle a 2 blocks re-org', async () => {
      ...
      expect(await heads.keys().all()).toStrictEqual([
