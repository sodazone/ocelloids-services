...
const _API_KEY = process.env.LILP_ROOT ?? ''
...
for (const subId of subIds) {
  const _ws = await agent.subscribe(subId, {
    onMessage: ({ payload }) => {
      ...
      console.log(
        `[${subId}] ${payload.inputs.assetSymbol} Reserve: ${reserve} Remote: ${remote} (Diff: ${Number(reserve) - Number(remote)})`,
      )
    },
